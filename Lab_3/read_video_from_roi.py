...
if len(sys.argv) < 3:
    print(
        "Select smaller ROI of a video file, and save the mean of each image channel to file, one column per color channel (R, G, B), each row corresponding to a video frame number."
    )
    print("")
    print(
        "Usage:\npython "
        + sys.argv[0]
        + " [path to input video file] [path to output data file]"
    )
    exit()
filename = sys.argv[1]
...
if not cap.isOpened():
    print(
        "Could not open input file. Wrong filename, or your OpenCV package might not be built with FFMPEG support. See docstring of this Python script."
    )
    exit()
...
while cap.isOpened():
    ...
    if count == 0:
        window_text = (
            "Select ROI by dragging the mouse, and press SPACE or ENTER once satisfied."
        )
        ROI = cv2.selectROI(
            window_text, frame
